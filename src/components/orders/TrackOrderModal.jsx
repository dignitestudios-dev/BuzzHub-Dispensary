...
const TrackOrderModal = ({ showModal, setShowModal, orderId, currentStatus, onStatusChange }) => {
  const [selectedStatus, setSelectedStatus] = useState(currentStatus);
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate()

  const statusList = ["Pending", "Approved", "Completed", "Rejected", "In Process", "Out for Delivery", "Ready", "Unknown"];
    ...
    showModal && (
      <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
        <div className="bg-white p-8 rounded-lg max-w-sm w-full">
          ...
          <div className="mb-4">
            <label htmlFor="status" className="block text-gray-600">Select Status</label>
            <select
              id="status"
