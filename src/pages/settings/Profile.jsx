  ...
  return (
    <div className="w-full text-black mx-auto p-6 bg-white">
      ...
      <div className="flex flex-col items-center text-center">
        <img
          src={dispensaryDetails.profilePicture || "https://i.pravatar.cc/?img=12"}  // Use a fallback image if profilePicture is not available
          alt="Profile"
          className="w-32 h-32 rounded-full object-cover border-4 border-gray-300"
        />
        <h2 className="text-2xl font-semibold mt-4">{dispensaryDetails.dispensaryName}</h2>
        <p className="flex items-center text-gray-600 text-sm mt-2">
          <FiMapPin className="mr-2" /> {dispensaryDetails.city}, {dispensaryDetails.state}
        </p>
        ...
        <p className="flex items-center text-gray-600 text-sm">
          <FiClock className="mr-2" /> {new Date(dispensaryDetails.openingHourTime).toLocaleTimeString()} - {new Date(dispensaryDetails.closingHourTime).toLocaleTimeString()}
        </p>
      </div>
      ...
      <div className="mt-4 bg-gray-50 p-6 rounded-lg shadow-sm">
        ...
        {dispensaryDetails.licenseFront && (
          <a href={dispensaryDetails.licenseFront} target="_blank" rel="noopener noreferrer" className="text-green-600">
            View Documents
          </a>
        )}
      </div>
